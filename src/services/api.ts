...
const EDGE_FUNCTION_URL = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-itinerary`;
  ...
  try {
    ...
    try {
      ...
      if (result.total_cost === undefined && result.itinerary) {
        result.total_cost = result.itinerary.reduce(
          (
            total: number,
            day: { parties: Array<{ ticket_price?: string }> },
          ) => {
            return (
              total +
              day.parties.reduce(
                (dayTotal: number, party: { ticket_price?: string }) => {
