...
function transformPartyRow(row: PartyRow): Party {
  ...
  const ticketPrice =
    row.ticket_tier_1 || row.ticket_tier_2 || row.ticket_tier_3 || "TBD";
 ...
 */
const PARTY_COLUMNS =
  "party_name, date, day, confirmed, start_time, end_time, venue, tags, link, ticket_tier_1, ticket_tier_2, ticket_tier_3, weekend_party";
...
export async function fetchPartiesByEvent(event: string): Promise<Party[]> {
  ...
  if (error) {
    throw new Error("Failed to load party data. Please try again.");
  }
...
export async function fetchAvailableEvents(): Promise<string[]> {
  ...
  const unique = [
    ...new Set(
      (data as { weekend_party: string }[]).map((r) => r.weekend_party),
    ),
  ];
  return unique.sort();
}
